        ...
        <div class="row">
            <div class="col-lg-12 margin-tb py-4">
                ...
                <div class="text-right mb-2">
                    <a class="btn btn-success" href="{{ route('payer.create1') }}"> <i
            ...
            <tbody>
                @foreach ($payer as $payer)
                    <tr>
                        ...
                        <td>{{ $payer->email_address }}</td>
                        <td><code>{{ $payer->username }}</code></td>
                        <td><code>{{ $payer->password }}</code></td>
                        <td>{{ $payer->status }}</td>
                        <td>
                            <form action="{{ route('payer.destroy', $payer->id) }}" method="Post">
                                <a class="btn btn-info" href="{{ route('payer.edit1', $payer->id) }}"><i class="fa-sharp fa-solid fa-marker fa-bounce fa-2xs" style="color: #ffffff;"></i></a>
                                @csrf
                                @method('DELETE')
